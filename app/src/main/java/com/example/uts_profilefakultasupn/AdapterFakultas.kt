...
class AdapterFakultas (val itemFakultas: List<DataFakultas>, val clickListener: (DataFakultas) -> Unit) :
RecyclerView.Adapter<RecyclerView.ViewHolder> (){
    ...
    class PartViewHolder (itemView: View): RecyclerView.ViewHolder(itemView) {
        fun bind(data : DataFakultas, clickListener: (DataFakultas) -> Unit) {
            itemView.list_gambar.setImageResource(data.gbrFak)
            itemView.list_fakultas.text = data.namaFak
